...
app.all('/test', (req, res) => {
    res.json({"msg":"모든 메서드 사용 가능!!"}) // JSON 형태로 변환 가능
});

// Router, Controller : 분배의 개념, 특정 요청이 왔을때 특정 모듈을 통해 일을 시키는 것
// Module, Service : 분배된 일을 실제로 처리하는 무언가
// View, Template : 사용자에게 보여주는 역할을 수행하는 UI
const router = require('./routers');
// /route 라는 요청이 오면 router 로 연결한다
app.use('/route',router);

// use 를 사용해 쓰는 모듈을 미들웨러 라고 한다.
